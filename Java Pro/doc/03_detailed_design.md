...
**スレッドセーフティ:**
- `private let lock = NSLock()` でコールバック内のデータ競合を防止。`didReceive` コールバックは任意スレッドから呼ばれるため、`lock.withLock {}` で保護。
...
### 5.15 LanguageManager

**ファイル**: `Services/LanguageManager.swift`  
**パターン**: `@MainActor @Observable final class` (シングルトン)

アプリ内の日英言語切替を一元管理するサービス。JSON ベースの文字列辞書（`strings_ja.json` / `strings_en.json`）から UI テキストを提供し、言語切替時に `ContentService` / `PracticeService` のコンテンツを再ロードする。
...
enum AppLanguage: String, CaseIterable, Codable {
    case japanese = "ja"
    case english = "en"
    var displayName: String    // "日本語" / "English"
    var subtitle: String       // "Japanese" / "英語"
    var bcp47Code: String      // "ja" / "en"
}
```
...
| プロパティ | 型 | 説明 |
|---|---|---|
| `currentLanguage` | `AppLanguage` | 現在の言語 (`private(set)`) |
| `isJapanese` | `Bool` (computed) | 日本語かどうか |

#### メソッド

| メソッド | 説明 |
|---|---|
| `setLanguage(_:)` | 言語切替。UserDefaults に保存し、文字列辞書をリロード。`ContentService` / `PracticeService` を再ロード |
| `l(_:)` | キーからローカライズ文字列を取得。キーが見つからない場合はキー自体を返却 |
| `l(_:, _:)` | フォーマットパラメータ付きローカライズ |
| `notificationStrings()` | 通知用文字列（タイトル・本文パターン）を一括取得 |

#### 永続化

| 項目 | 値 |
|---|---|
| UserDefaults キー | `"appLanguage"` |
| デフォルト値 | `"ja"` (日本語) |
| 通知名 | `LanguageManager.languageDidChangeNotification` |
...
### 8.5 GlossaryPopupView

**ファイル**: `Views/Components/GlossaryPopupView.swift`

レッスン本文中の `[[用語]]` リンクをタップした際にハーフシート形式で表示される用語ポップアップ。
...
**UI 構成:**
- 先頭文字アイコン（円形背景）+ 用語名 + 読み仮名
- 定義テキスト（本文）
- 関連レッスンチップ（水平スクロール、`ContentService.shared.getLesson(id:)` で取得）
- `.presentationDetents([.height(280), .medium])` でシートサイズ制御
- スプリングアニメーション付きフェードイン
...
| ファイル | レコード数 | 説明 |
|---|---|---|
| `courses_index.json` / `courses_index_en.json` | 34 コース | コースインデックス（日英） |
| `ch01_introduction.json` 〜 `ch38_spring_boot.json` | 34 ファイル × 日英 = 68 / 169 レッスン / 575 クイズ | チャプターコンテンツ |
| `exam_questions_se11_silver_1.json` 等 | 8 ファイル × 日英 = 16 / 640 問 | 模擬試験問題 |
| `glossary.json` / `glossary_en.json` | 202 語 | 用語辞典 |
| `practice_exercises.json` / `practice_exercises_en.json` | 39 チャプター / 108 問 + 5 セクション | 実践演習 + 環境構築ガイド |
| `strings_ja.json` / `strings_en.json` | — | UI 文字列辞書 |
| **合計** | **92 ファイル** | 日英各 46 ファイル |
...
| 17 | ch19_java17_features | Java SE 17新機能 | 6 |
...
| 2.1.0 | 2026-04-14 | LanguageManager サービス追加 (§5.15)。GlossaryPopupView コンポーネント追加 (§8.5)。CrashReportService に NSLock スレッドセーフティ記述追加。RootView 学習タイマーを Timer → Task パターンに修正。リソースファイル数を英語版含む 92 ファイルに更新。ch19 コース ID を ch19_java17_features に修正 |
